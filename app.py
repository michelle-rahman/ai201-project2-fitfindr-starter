...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # 4. On early exit, surface the error in the first panel only.
    if session["error"]:
    ...
    if session.get("loosened"):
        listing_text += "\n\nNote: " + "; ".join(session["loosened"]) + " to find a match."

    return listing_text, session["outfit_suggestion"], session["fit_card"]


def _format_listing(item: dict) -> str:
    """Render the selected listing dict as readable text for the UI panel."""
